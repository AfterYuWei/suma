...
- [x] Secure TCP and Compose: mTLS by default, loopback-only plaintext, encrypted TLS credentials, explicit absolute remote binds, and private temporary TLS/registry configuration with cleanup. Verification: security tests and compose multinode runner cleanup tests pass (`go test ./...` green on 2026-08-30).
