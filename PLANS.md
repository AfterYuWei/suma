...
- [x] External Project cleanup: expose deletion from both the Project list and detail view without requiring takeover, and retain the blocked-takeover cleanup path. The Project-level destructive cleanup Task requires the complete Project Name, force-removes only exact-label Project containers, removes only Project-owned networks, and preserves bind paths and all volumes by default. Project-owned named-volume deletion is an independent high-risk opt-in, uses non-force Docker removal, and has a distinct Audit action. Verification: capability/service/adapter/authenticated-HTTP tests, isolated real-Docker preserve/delete-volume smoke, `go test ./...`, `go build ./...`, `npm run lint`, `npm run typecheck`, production build, and demo build.
