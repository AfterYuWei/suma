...
export function TasksPage() {
  ...
  const query = useQuery({ queryKey: ['tasks'], queryFn: () => api<Task[]>('/tasks'), refetchInterval: 2_000 })
  const prune = useMutation({ mutationFn: () => api('/system/prune', { method: 'POST', body: JSON.stringify({ confirm: 'PRUNE' }) }), onSuccess: () => client.invalidateQueries({ queryKey: ['tasks'] }) })
  ...
  return <ResourceFrame eyebrow={t('operations')} title={t('tasks')} detail={language === 'zh-CN' ? '长时间运行的 Docker 与 Compose 操作' : 'Long-running Docker and Compose operations'} action={<button onClick={() => void startPrune()} className="h-8 rounded-md border border-red-900/60 bg-surface px-3 text-xs text-red-400">{t('systemPrune')}</button>}>{query.isPending ? <LoadingState label={language === 'zh-CN' ? '正在加载任务' : 'Loading tasks'} /> : <div className="divide-y divide-border border-y border-border">{query.data?.map((row) => <TaskRow key={row.id} row={row} />)}</div>}</ResourceFrame>
}
