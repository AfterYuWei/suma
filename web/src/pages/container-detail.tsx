...
export function ContainerDetailPage() {
  ...
  const query = useQuery({ queryKey: ['container', containerId], queryFn: () => api<ContainerDetail>(`/containers/${containerId}`) })
  const action = useMutation({ mutationFn: (name: string) => api(`/containers/${containerId}/${name}`, { method: 'POST' }), onSuccess: () => { setMenu(false); client.invalidateQueries({ queryKey: ['container', containerId] }); client.invalidateQueries({ queryKey: ['containers'] }) } })
  const rename = async () => { const name = await promptDialog({ title: t('renameContainer'), confirmLabel: t('save'), input: { label: t('newContainerName'), initialValue: query.data?.name } }); if (!name) return; await api(`/containers/${containerId}`, { method: 'PATCH', body: JSON.stringify({ name }) }); await client.invalidateQueries({ queryKey: ['container', containerId] }); setMenu(false) }
  const remove = async () => { const name = query.data?.name ?? containerId; if (!await confirmDialog({ title: t('removeContainer'), description: t('removeContainerDescription', { name }), confirmLabel: t('remove'), danger: true })) return; await api(`/containers/${containerId}`, { method: 'DELETE' }); void navigate({ to: '/containers' }) }
  ...
  if (!query.data) return <p className="py-12 text-sm text-red-400">{zh ? '未找到容器。' : 'Container not found.'}</p>
  ...
  return <div>
    ...
    {tab === 'Logs' && <div className="py-6"><LogViewer containerId={containerId} /></div>}{tab === 'Terminal' && <div className="py-6"><TerminalView containerId={containerId} /></div>}{tab === 'Stats' && <div className="py-6"><StatsView containerId={containerId} /></div>}{tab === 'Inspect' && <pre className="my-6 max-h-[60vh] overflow-auto rounded-md border border-border bg-[#090b0d] p-4 text-[11px] text-text-muted">{JSON.stringify(row, null, 2)}</pre>}
  </div>
}

function MenuAction({ label, icon: Icon, run, danger = false }: { label: string; icon: typeof Play; run: () => void; danger?: boolean }) { return <button onClick={run} className={`flex h-8 w-full items-center gap-2 rounded px-2 text-xs hover:bg-surface-hover ${danger ? 'text-red-400' : ''}`}><Icon className="size-3.5" />{label}</button> }
