  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="sm:max-w-lg">
      <DialogHeader>
        ...
        <DialogDescription>{zh ? 'SUMA 可以根据安全可访问的 Compose 配置或 Docker 运行态生成托管配置草稿，也可以由你直接填写 compose.yml 完成接管。' : 'SUMA can generate a managed configuration draft from safely accessible Compose files or Docker runtime metadata, or you can write the compose.yml directly.'}</DialogDescription>
      </DialogHeader>
      <ul className="list-disc space-y-2 pl-5 text-sm text-muted-foreground">
        ...
        <li>{zh ? '自动生成的草稿不会保留原 YAML 的注释、锚点、多文件结构和原始变量表达式；如需完整保留，可直接粘贴原始 compose.yml 与 .env。' : 'The generated draft cannot preserve comments, anchors, multi-file layout, or original variable expressions; paste the original compose.yml and .env directly to keep them.'}</li>
