...
import { Container, GitPullRequest, KeyRound, Layers3, Package, Search, Search as Command } from 'lucide-react'
...
export function CommandPalette({ open, close }: { open: boolean; close: () => void }) {
  ...
  const results = useMemo<Result[]>(() => {
    const values: Result[] = [
      ...
      { id: 'continuous-delivery', label: zh ? '打开持续交付' : 'Open continuous delivery', detail: zh ? 'Git 发布与回滚' : 'Git releases and rollback', type: zh ? '操作' : 'Actions', icon: GitPullRequest, run: () => { close(); void navigate({ to: '/continuous-delivery' }) } },
      ...
      { id: 'authentication', label: zh ? '打开认证中心' : 'Open Authentication Center', detail: zh ? 'Git 与镜像仓库凭据' : 'Git and registry credentials', type: zh ? '操作' : 'Actions', icon: KeyRound, run: () => { close(); void navigate({ to: '/authentication' }) } },
    ]
    containers.data?.forEach((row) => {
      ...
      values.push({ id: `restart-${row.id}`, label: zh ? `重启 ${row.name}` : `Restart ${row.name}`, detail: zh ? '容器操作' : 'Container action', type: zh ? '容器操作' : 'Container actions', icon: Container, run: () => { close(); void api(nodePath(nodeID, `/containers/${row.id}/restart`), { method: 'POST' }) } })
      if (row.state === 'running') values.push({ id: `stop-${row.id}`, label: `${zh ? '停止' : 'Stop'} ${row.name}`, detail: zh ? '容器操作' : 'Container action', type: zh ? '容器操作' : 'Container actions', icon: Container, run: () => { void confirmDialog({ title: zh ? `停止 ${row.name}？` : `Stop ${row.name}?`, description: zh ? '容器将收到正常停止信号。' : 'The container will receive a graceful stop signal.', confirmLabel: zh ? '停止' : 'Stop' }).then((confirmed) => { if (confirmed) { close(); void api(nodePath(nodeID, `/containers/${row.id}/stop`), { method: 'POST' }) } }) } })
    })
    projects.data?.forEach((row) => {
      values.push({ id: `project-${row.name}`, label: row.name, detail: zh ? 'Compose 项目' : 'Compose project', type: 'Compose', icon: Layers3, run: () => { close(); void navigate({ to: '/compose/$projectName', params: { projectName: row.name } }) } })
    })
    deliveries.data?.forEach((row) => values.push({ id: `delivery-${row.name}`, label: row.name, detail: zh ? '持续交付项目' : 'Continuous delivery project', type: zh ? '持续交付' : 'Continuous Delivery', icon: GitPullRequest, run: () => { close(); void navigate({ to: '/continuous-delivery/$projectName', params: { projectName: row.name } }) } }))
  ...
  }, [containers.data, projects.data, deliveries.data, images.data, search, close, navigate, zh, nodeID])
