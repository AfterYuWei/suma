...
func TestComposeProjectIdentityIncludesBackendAndEngineScope(t *testing.T) {
	...
	if !HasCapability(value.Capabilities, CapabilityTakeover) || HasCapability(value.Capabilities, CapabilityEdit) {
		t.Fatalf("external capabilities = %#v", value.Capabilities)
	}
	managed := ComposeSummary("node-a", "shop", "managed", "running", true)
	if !HasCapability(managed.Capabilities, CapabilityEdit) || HasCapability(managed.Capabilities, CapabilityTakeover) {
