...
- Docker Swarm's official object is a Swarm Stack. Future SUMA support would map it to `Project backend=swarm`; no Swarm adapter, runner, API, scheduler, task model, or menu is implemented now.
...
The first user becomes administrator. Passwords use bcrypt. Authentication uses opaque, hashed server-side sessions in HttpOnly cookies. Optional TOTP two-factor authentication keeps its seed AES-GCM encrypted, stores recovery codes only as bcrypt hashes, rejects replayed time steps, and creates a session only after a short-lived, attempt-limited login challenge succeeds. Passwordless WebAuthn Passkeys use random opaque user handles, discoverable credentials, required user verification, exact origin/RP binding, and hashed one-time ceremonies; authenticator private keys never reach SUMA. State-changing user routes require an authenticated session. Destructive commands require explicit client confirmation and are audited server-side. Secrets are excluded from logs and sensitive environment keys are masked in API/UI representations.
